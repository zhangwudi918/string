...
public class SaveMap {
	...
	public static synchronized String savemessage(String message) {
		...
		boolean check = lexicon.containsKey(message);
		if (check) {
			return "该词已存在，序号为：" + lexicon.get(message);
		} else {
	...
	/*	Set<Entry<String, String>> set = lexicon.entrySet();
		for (Entry<String, String> e : set) {
			...
			String value = sb.append(table[len % 62]).reverse().toString();
			lexicon.put(message, value);
			return "提交成功! 序号为：" + value;
